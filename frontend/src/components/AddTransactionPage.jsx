...
import { HOME_SURFACE, HOME_DIVIDER, HOME_TEXT, HOME_MUTED, HOME_INCOME, HOME_EXPENSE, HOME_ACCENT, ACCENT, CATEGORY_ACCENT } from "./categoryVisuals";
...
// The 3-card chooser that "Add Transaction" opens to, on the same main-
// content takeover Tools use (toolMode "add" in Dashboard.jsx). Clicking a
// card focuses it into a wide centered panel with its actual content; the
// other two collapse away rather than shrinking beside it, replaced by
// left/right arrows on the panel that cycle between them, and clicking the
// surrounding empty space returns to the 3-card grid.
export default function AddTransactionPage({ onSaved, onImported }) {
  ...
  // FLIP transition (First-Last-Invert-Play): clicking a card records its
  // on-screen rect, then once the focused panel has rendered in its final
  // (centered, full-size) position, we measure that and animate the panel
  // *from* the card's old rect back to identity via a plain transform -
  // reads as the card growing into the panel instead of the grid and the
  // panel just crossfading past each other. Only fires opening from the
  // grid (a source rect exists) - cycling between focused cards via the
  // arrows has no "card" to grow from, so that path skips this.
  ...
  return (
    <div>
      ...
      {/* The 3-card grid and the focused panel are both always mounted -
          toggled with display, not a ternary - so an in-progress Batch
          draft or an uploaded Import file survives clicking off, cycling
          with the arrows, or reopening the "Add Transaction" tool later
          (same "mount once, keep it" convention Dashboard.jsx uses for its
          other tool panels). */}
      <div
        style={{
      ...
      >
        ...
        >
          ...
          >
            {/* Each key's content restarts add-panel-slide whenever it flips
                from display:none to visible - a browser quirk that works in
                our favor here, since it means both the arrow-cycle swap and
                the very first open replay the exact same slide/fade the rest
                of the app uses when changing pages (Dashboard.jsx's own
                tool-page-in keyframe - duplicated here rather than shared
                since it's a plain <style> tag, not a module). */}
            {ORDER.map((key) => (
...
function SingleForm({ onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Clears and stays focused on Single rather than bouncing back to the
      // 3-card grid - adding several one after another is the common case.
  ...
  // Underline-only fields - fully transparent, no fill box at all, just a
  // bottom line that brightens to the category color on focus, same "grows
  // into the accent color" idea as the Navbar search bar.
