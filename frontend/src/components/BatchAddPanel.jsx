...
import { HOME_SURFACE, HOME_DIVIDER, HOME_TEXT, HOME_MUTED, HOME_EXPENSE, CATEGORY_ACCENT, CATEGORY_ICON } from "./categoryVisuals";
...
// A vertical list of draft cards, not a spreadsheet table - each row gets
// its own category-colored icon tile plus room for its fields to breathe,
// which the old table's fixed columns didn't allow. Add/remove logic and
// the onSaveStateChange contract are unchanged from the table version.
export default function BatchAddPanel({ active, onSaveStateChange, onSaved, onCancel }) {
  ...
  return (
    <div style={{ color: text, minHeight: 420, display: "flex", flexDirection: "column" }}>
      ...
      {/* Two-column grid of squarish cards, not a wide table row per draft -
          keeps a single entry from reading as a tiny sliver at the top of a
          panel that's otherwise this roomy. */}
