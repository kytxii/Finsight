...
// Desktop's first pass at Installments - mobile-only until now (MobileInstallments.jsx).
// Same math/gauge/API as mobile (installmentMath.js, InstallmentGauge).
// `desktop` renders a full main-content-page layout (stat header + a card
// grid with a real gauge per card) instead of the compact sidebar list.
...
// payments_made * monthly_payment (not period_months - payments_made months'
// worth) so this stays exact even when monthly_payment carries rounding from
// an unevenly-divisible total - mirrors MobileInstallments.jsx exactly.
...
export default function InstallmentsPanel({ desktop = false, addSignal, onSaved }) {
  ...
  // Fetched once for the whole list, same figure each row's Impact stat and
  // the add/edit form's gauge preview both read from.
  useEffect(() => {
    let cancelled = false;
    getSpendableSurplus()
  ...
  // for its own header "+" button - Dashboard.jsx's page-level header lives
  ...
  // Delete lives in the edit form now, not a button on the main card - close
  // first (same order MobileInstallments' own sheet delete uses) so the form
  // isn't left open pointing at a row that's about to disappear.
  ...
  // Shared add/edit form - same JSX whether it's inline in the sidebar list
  // or a grid cell on the desktop page.
  const formNode = (
  ...
  // ── Desktop: stat header + card grid ──────────────────────────────────────
  // Each card leads with the number that actually matters day-to-day (the
  // monthly payment, not the gauge) and a slim colored bar for impact instead
  // of the semicircle gauge - that reads fine as a big centerpiece in the
  // add/edit form, but cramped next to a 2x2 stat grid in a ~280px card it
  // just made everything feel tight and unbalanced.
  if (desktop) {
