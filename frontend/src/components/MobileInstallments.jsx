...
// Matches the entry animation duration used by both sheets below, so the exit
// transition (which rides a plain CSS transition rather than the keyframe)
// takes the same time to close as it did to open.
const EXIT_MS = 260;
...
// Capitalizes the first letter of each word as it's typed - only touches a
// lowercase letter right after the start of the string or a space, so
...
// payments_made * monthly_payment rather than period_months - payments_made
// months' worth, so this stays exact even if monthly_payment carries rounding
// from an unevenly-divisible total (flat division, see compute_monthly_payment).
// Falls back to 0 defensively - demo-mode rows created before payments_made
// existed on the seed data won't have it.
...
function EditSheet({ draft, setDraft, mode, saving, error, onCancel, onSave, onDelete, deleteConfirm }) {
  ...
  // Purely local math - no network round trip, so these recompute live on
  // every keystroke as the amount changes.
  ...
  // Fetched once per sheet open, independent of pickMode, so switching tabs
  // is instant. Only used for the "By payment" tiles' safety coloring -
  // failure is silent (tiles fall back to neutral, no error shown to the user).
  // Cash actually free to allocate right now (balance + income before bills
  // reset, minus the reserve floor) - not a projected monthly savings figure.
  ...
  // Target-payment field (By payment mode) is just another view over the same
  // draft.period_months - typing an amount picks the nearest whole-month term
  // that covers it (ceil), so the actual schedule is always shown alongside
  // whatever was typed rather than silently diverging from it.
  ...
  // The header gauge only appears once there's an actual payment to plot
  // against real available cash - before that, null/red states from
  // computeGaugeStatus would read as "this is unaffordable" when it's really
  // just "nothing entered yet", which is misleading in a corner icon with no
  // supporting text. It re-derives on every keystroke since preview and
  // savings.value already do, so it sweeps smoothly as amount/term/payment change.
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Drag region covers the notch and title row only - the fields and
            tile grid below need normal touch handling for typing/tapping. */}
        ...
        {/* Spells out what the gauge's percentage is a percentage *of*. Without
            this the header gauge is a bare colour with no visible denominator,
            which is the one thing a user needs to judge whether the reading
            means anything. "Left" is deliberately not clamped at zero - a
            negative here is the clearest possible signal that the payment
            doesn't fit, and hiding it would flatter the number. */}
        {gaugeInfo && (
        ...
        {/* Pushed down further and left as a plain-text fallback - swipe-to-delete
            on the row itself (SwipeableRow) is the primary way to remove one;
            this exists for when the sheet is already open anyway. */}
        {mode === "edit" && (
...
// The gauge that used to live here moved into EditSheet's header, where it's
// live and contextual instead of a separate fetch-and-wait step. This button
// is left in place as the future entry point for AI-powered insights (a
// follow-up feature, not built yet) - for now it just says so.
...
export default function MobileInstallments({ onSaved, openAddSignal }) {
  ...
  // Mirrors the same list-fetch on mount every other panel here uses - but unlike
  // those, failure sets loadFailed instead of swallowing the error: silently
  // landing on the "No installments yet" empty state on a failed fetch looks
  // identical to actually having none, which reads as data loss when it's really
  // a network/auth hiccup and the rows are still sitting in the database.
  useEffect(loadInstallments, []);

  // Fetched once for the whole list (not per row) to drive each card's Rate
  // stat - same figure EditSheet fetches independently for its own preview.
  // Failure is silent: rows just show "—" for Rate instead of an error, same
  // treatment as the "By payment" tiles when this isn't available.
    ...
    <div style={{ flex: 1, overflowY: "auto", overscrollBehavior: "contain", padding: "16px 16px 32px", color: HOME_TEXT }}>
      ...
      {loading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[...Array(4)].map((_, i) => (
            <Skel key={i} h={60} style={{ borderRadius: 14, opacity: 1 - i * 0.15 }} />
          ))}
        </div>
      ) : loadFailed ? (
