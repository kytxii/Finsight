...
// Mobile transaction detail modal (#27). Replaces the old scroll-to-row-and-
// highlight flow when a transaction is selected from search, and also serves
// as mobile's general-purpose edit surface (Home, category pages, Tips,
// Activity all route their swipe-to-edit here now) - the fresh dark-themed
// component #27's issue called for, not an extension of the shared
// (desktop-facing) EditTransactionModal. Same bottom-sheet conventions as
// MobileRecurring's EditSheet: drag-to-dismiss via the caller, tap-again-to-
// confirm delete, HOME_* palette throughout.
  ...
  return (
    <>
      ...
      >
        ...
        {/* Date renders its own compact icon + short label instead of the
            native input's own content (locale segments + calendar-icon
            affordance) - that's what was clipping at half width. The real
            date input is still here, just invisible and stretched over the
            whole box, so tapping anywhere on it opens the native OS picker
            as normal; what's visible underneath is just for display. */}
        ...
        {/* Extra top margin + divider separates Delete from Cancel/Save so it
            reads as a distinct, less-reachable action rather than a third
