...
// Desktop-only stat card - same visual language as the dashboard's overview
// cards (filled surface, no border - label caption + big value), scaled down
// to fit four across a header row instead of the whole overview grid.
...
// `desktop` swaps the sidebar's settings/list *toggle* for a permanent
// two-column layout (history left, schedule/balance/reserve settings right)
// plus a stat header - same state and handlers throughout, just laid out
// differently now that there's a full page width to use instead of a ~400px
// sidebar column.
export default function PaychecksPanel({ mobile = false, desktop = false, onSaved }) {
  ...
  const [view, setView] = useState("list"); // "list" | "settings" - sidebar mode only
  ...
  async function load() {
    ...
    try {
      ...
      // Loading state below (and the loadFailed-less empty states per section)
      // covers the failure case - nothing to surface here.
  ...
  // Next paycheck (today or later) for the desktop stat header - first
  // upcoming/pending entry in date order, whether or not it has an amount yet.
  ...
  // ── Shared sections - same JSX whether they end up in the sidebar's
  // toggled "Settings" view or desktop's permanent right-hand column. ───────
        ...
        {loading ? (
          ...
          <div className="grid" style={{ gridTemplateColumns: "1fr 360px", gap: 24, alignItems: "start" }}>
            <div className="rounded-2xl px-6 py-5" style={{ backgroundColor: bg, display: "flex", flexDirection: "column" }}>
              ...
              {/* Contained scroll region with the sidebar's own infinite-scroll
                  pagination (handleScroll/visibleCount), not the whole page -
                  otherwise a year of biweekly paychecks makes the entire
                  page scroll instead of just this list. */}
