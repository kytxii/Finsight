...
export default function ImportPanel({ active, onSaveStateChange, onSaved, onCancel }) {
  // Shared between desktop and mobile - both use the same dark palette now.
  ...
  // Excluded rows are already opted out, so they don't count toward "needs a decision."
  ...
  // Cycles a symbol while AI cleanup runs, same treatment as the auth-page spinner.
  useEffect(() => {
    if (!aiCleanupLoading) return;
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden", color: text, borderTop: `1px solid ${border}` }}>
      {loading && (
        <div style={{ position: "relative", padding: "36px 20px 44px" }}>
          ...
          `}</style>
          {/* Purely decorative - the real progress signal is the phrase in the corner. */}
      ...
      {!loading && !loadError && sourceFormat && (
        <>
          ...
          {/* Rows needing a decision surface above a divider first */}
                    ...
                    <textarea
                      ...
                      onBlur={() => {
                        ...
                        if (r.name.trim() === "" && original.trim() !== "") {
                          // Restores the name if it's cleared and left blank.
